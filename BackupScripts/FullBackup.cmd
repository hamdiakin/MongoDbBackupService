...
echo "%mongoDumpPath%" --uri="%mongoDbAddress%" --db=%databaseName% --gzip --out="%backupDir%" > "%backupDir%\full-backup-command.log"

:: Run the backup command and capture output
"%mongoDumpPath%" --uri="%mongoDbAddress%" --db=%databaseName% --gzip --out="%backupDir%" > "%backupDir%\full-backup.log" 2>&1
